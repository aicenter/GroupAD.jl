...
function results_dataframe_row_at_val(target::String, verb = false)
    data = load(target)
    seed = data[:seed]
    no_anomalies = sum(data[:val_labels])
...
function results_dataframe_row_at_val_over_seeds(target::String; verb = false, seeds = 50)
    ...
    for no in val_an
        if no < no_anomalies
            # calculate the scores (AUC-ROC, AUC-PR etc.)
            # it should be enough to calculate only validation scores
            # not train, not test, only validation
            if all(isnan.(scores))
                if verb
                    @info "score is NaN"
                end
            ...
            push!(results_val, [no, mean(AUC), mean(AUPR)])
        end
    end
...
function results_dataframe_at_val_over_seeds(target::Array{String,1})
	j = 1
	df = []
	for i in 1:length(target)
		r = results_dataframe_row_at_val_over_seeds(target[i])
		if !isnothing(r)
			df = results_dataframe_row_at_val_over_seeds(target[i])
			break
		end
		j += 1
	end
...
function results_dataframe_at_val_over_seeds(target::String)
    files = GroupAD.Evaluation.collect_scores(target)
    results_dataframe_at_val_over_seeds(files)
end
