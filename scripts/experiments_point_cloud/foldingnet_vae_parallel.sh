...
LOG_DIR="${HOME}/MIL/Logs/FoldingNetVAE-parts/MNIST/"
...
do 
    ...
    do 
        sbatch \
        --output="${LOG_DIR}/MNIST-${RANDOM_SEED}_%j_${i}_${j}.out" \
        ./foldingnet_vae.sh -$i ${DATASET} -$j ${METHOD} ${CONTAMINATION} ${RANDOM_SEED}
