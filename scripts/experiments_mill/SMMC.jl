...
function sample_params()
    ...
    rand() > 0.5 ? c = rand(Uniform(0,3)) : c = rand(Uniform(3,100))
...
function fit(data, parameters)
	...
	model = GroupAD.Models.SMMCModel(parameters...)
    @info "Models created."
...
function edit_params(data, parameters)
    ...
    if parameters.distance == "MMD"
        ...
        m = 1/median(M)
        γnew = sample(0.8m:0.1m:1.2m)
        parameters = merge(parameters, (γ = γnew,))
    end

    M = pairwise(TotalVariation(), length.(Xtrain))
    v = 1/median(M)
    cnew = sample(0.9v:0.1v:1.1v)
    parameters = merge(parameters, (c = Float32(cnew),))
